...
class TestStructParser(unittest.TestCase):
    ...
    def test_structure_declaration_no_error(self):
        tokens_list = [Token('struct', '', 1), Token('id', 1, 1), Token('left_brace', '', 1),
                       Token('newline', '', 1), Token('var', '', 2), Token('id', 2, 2),
                       Token('assignment', '', 2), Token('number', 3, 2), Token('newline', '', 2),
                       Token('right_brace', '', 3), Token('newline', '', 3)]
        table = SymbolTable()
        table.entry("my_struct", "var", "variable")
        table.entry("a", "int", "variable")
        table.entry("a", "int", "constant")

        opcode, _, _ = struct_declaration_statement(tokens=tokens_list, i=1, table=table)
