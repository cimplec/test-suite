...
class TestVariableParser(unittest.TestCase):
    ...
    def test_check_ptr_true_case(self):
        tokens_list = [Token("multiply", "", 1), Token("multiply", "", 1), Token("newline", "", 1)]
    ...
    def test_var_statement_array_expected_integer_size_of_array(self):
        tokens_list = [Token("var", "", 1), Token("id", 1, 1), Token("left_bracket", "", 1),
                       Token("number", 2, 1), Token("right_bracket", "", 1), Token('assignment', '', 1),
                       Token('left_brace', '', 1), Token('number', 3, 1), Token('comma', '', 1), ]
        table = SymbolTable()
    ...
    def test_var_statement_array_assign_no_error(self):
        tokens_list = [Token("var", "", 1), Token("id", 1, 1), Token("left_bracket", "", 1),
                       Token("number", 2, 1), Token("right_bracket", "", 1), Token('assignment', '', 1),
                       Token('left_brace', '', 1), Token('number', 3, 1), Token('comma', '', 1),
                       Token('number', 4, 1), Token('right_brace', '', 1), Token('newline', '', 1)]
        table = SymbolTable()
        table.entry("a", "var", "variable")
        ...
        opcode, _, _ = var_statement(tokens=tokens_list, i=1, table=table, func_ret_type={})

        self.assertEqual(opcode, OpCode('array_assign', 'a---2---{1,2}', 'int'))

    def test_var_statement_array_invalid_syntax_for_declaration(self):
        tokens_list = [Token("var", "", 1), Token("id", 1, 1), Token("left_bracket", "", 1),
                       Token("number", 2, 1), Token("right_bracket", "", 1), Token("plus_equal", "", 1),
                       Token('newline', '', 1)]
        table = SymbolTable()
        table.entry("a", "var", "variable")
        table.entry("2", "int", "constant")
    ...
    def test_var_statement_ptr_assign(self):
        ...
        table = SymbolTable()
        table.symbol_table = {1: ['b', 'var', 'variable'], 2: ['1', 'int', 'constant'], 3: ['a', 'var', 'variable']}
    ...
    def test_var_statement_var_assign(self):
        tokens_list = [Token('var', '', 1), Token('id', 1, 1), Token('assignment', '', 1),
                       Token('number', 2, 1), Token('newline', '', 1)]
        table = SymbolTable()
        table.symbol_table = {1: ['b', 'var', 'variable'], 2: ['1', 'int', 'constant'], 3: ['a', 'var', 'variable']}
    ...
    def test_var_statement_ptr_no_assign(self):
        tokens_list = [Token('var', '', 1), Token('multiply', '', 1), Token('id', 1, 1), Token('newline', '', 1)]
        table = SymbolTable()
        table.symbol_table = {1: ['b', 'var', 'variable'], 2: ['1', 'int', 'constant'], 3: ['a', 'var', 'variable']}
